    ...
    private void OnAnimatorIK(int layerIndex)
    {
...

        {
            ...
            else
            {
                ...
                SetTransPos(BodyPart.Left_LowerArm, IK_Elbow(GetTransPos(BodyPart.Left_Hand), Shoulder, PartDistance.GetVaule(2), PartDistance.GetVaule(3)));

                //GetPool("LeftHand").transform.position = GetTransPos(BodyPart.Left_Hand);
                //GetPool("Shoulder").transform.position = Shoulder;
                //GetPool("IK Elbow_L").transform.position = GetTransPos(BodyPart.Left_LowerArm);

                Vector3 W_Hand = GetTransPos(BodyPart.Left_Hand);
                W_Hand.z = Shoulder.z - (HandZ_Limit_Mutiply.Evaluate(Xrate) * HandZ_Limit);
                //GetPool("Z Limit Hand").transform.position = W_Hand;

                Vector3 IKedHand = GetTransPos(BodyPart.Left_LowerArm) + (W_Hand - GetTransPos(BodyPart.Left_LowerArm)).normalized * PartDistance.GetVaule(3);
                //GetPool("IKed Hand").transform.position = IKedHand;

                SetTransPos(BodyPart.Left_Hand, IKedHand);
...

        {
            ...
            else
            {
                ...
                SetTransPos(BodyPart.Right_LowerArm, IK_Elbow(GetTransPos(BodyPart.Right_Hand), Shoulder, PartDistance.GetVaule(4), PartDistance.GetVaule(5)));
                ...
                Vector3 IKedHand = GetTransPos(BodyPart.Right_LowerArm) + (W_Hand - GetTransPos(BodyPart.Right_LowerArm)).normalized * PartDistance.GetVaule(5);
                //GetPool("IKed Hand").transform.position = IKedHand;

                SetTransPos(BodyPart.Right_Hand, IKedHand);
...
*/
    ...
    public Vector3 IK_Elbow(Vector3 Hand, Vector3 Shoulder , float UpperArm, float LowerArm)
    {
        //가상 UpperArm (HE') = (H - S).Distance * (LowerArm / (LowerArm + UpperArm));
        //가상 Elbow (E') = H + ((H - S).normalized * - 1 * (LowerArm / (LowerArm + UpperArm) * (H - S).Distance);
        //E (2D) = E' + ((H - S).OrthNormal * LowerArm * Sin( Acos( HE' / LowerArm ) ));

        //if (S.x > 0)
        //  E = New Vector3 (Max(E.x , S.x), E.y, Max(Abs(S.x) - Abs(E.x), 0));
        //if (S.x < 0)
        //  E = New Vector3 (Min(E.x , S.x), E.y, Max(Abs(S.x) - Abs(E.x), 0));

        float LocalHandDistance = (Hand - Shoulder).magnitude;
        float VirtualUpperArm = LocalHandDistance * (LowerArm / (LowerArm + UpperArm));
        Vector3 VirtualElbow = Hand + ((Hand - Shoulder).normalized * -1 * (LowerArm / (LowerArm + UpperArm)) * LocalHandDistance);

        Vector3 HandNormal = (Hand - Shoulder).normalized;
        Vector3 ElbowOrtho = Vector3.down;
        Vector3.OrthoNormalize(ref HandNormal, ref ElbowOrtho);

        Vector3 Elbow_2D = VirtualElbow + (ElbowOrtho * LowerArm * Mathf.Sin(Mathf.Acos(VirtualUpperArm / LowerArm)));

        if (Shoulder.x > 0)
        {
            return new Vector3(Mathf.Max(Elbow_2D.x, Shoulder.x), Elbow_2D.y, Elbow_2D.z + Mathf.Max(Mathf.Abs(Shoulder.x) - Mathf.Abs(Elbow_2D.x), 0));
        }else
        {
            return new Vector3(Mathf.Min(Elbow_2D.x, Shoulder.x), Elbow_2D.y, Elbow_2D.z + Mathf.Max(Mathf.Abs(Shoulder.x) - Mathf.Abs(Elbow_2D.x), 0));
        }
    }
}
